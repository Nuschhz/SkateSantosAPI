const validateRentalStatus = (req, res, next) => {
  const { status } = req.body;
  const validStatuses = ["canceled", "done"]; // Valores válidos para o status
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "O status deve ser um número válido (0 para 'canceled', 1 para 'done').",
    });
  }
...
module.exports = validateRentalStatus;
